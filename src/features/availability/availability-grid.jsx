...
export function AvailabilityGrid({ slots }) {
  ...
  const firstAvailable = normalizedSlots.find((slot) => slot.state === "available")?.time || null;
      ...
      <div className="overflow-x-auto pb-2">
        <div className="grid min-w-[760px] grid-cols-6 gap-3 md:min-w-0 p-4">
          {normalizedSlots.map((slot) => (
            <motion.div key={slot.time} whileHover={slot.state === "available" ? { y: -3 } : undefined} whileTap={slot.state === "available" ? { scale: 0.97 } : undefined}>
              <Button
                fullWidth
...
function Legend({ color, label }) {
  return <span className="inline-flex items-center gap-2"><span className={cn("size-2 rounded-full", color)} />{label}</span>;
}
