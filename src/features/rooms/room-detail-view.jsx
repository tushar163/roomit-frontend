  ...
  if (!room) {
    return (
      <Card className="glass-panel rounded-2xl p-8">
        ...
        <p className="mt-2 text-sm text-zinc-500">Check the room id or seed rooms in the backend.</p>
      </Card>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <section className="relative overflow-hidden rounded-3xl border border-white/10">
        <div className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: `url(${room.image || fallbackImage})` }} />
        ...
        <div className="relative min-h-[420px] max-w-3xl p-6 text-white sm:p-10">
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <Chip><Users size={15} /> {room.capacity} people</Chip>
            <Chip><Wifi size={15} /> Hybrid ready</Chip>
            <Chip><Monitor size={15} /> 4K display</Chip>
      ...
      <section className="grid gap-4 xl:grid-cols-[1fr_340px]">
        <Card className="glass-panel rounded-2xl p-5">
          <div className="mb-5 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="text-sm text-zinc-500 dark:text-zinc-400">Daily 30-minute slots for {room.name}.</p>
            </div>
            <Input type="date" value={date} onChange={(event) => setDate(event.target.value)} variant="bordered" className="max-w-44" aria-label="Availability date" />
          </div>
          {loading ? <div className="h-52 animate-pulse rounded-2xl bg-white/10" /> : <AvailabilityGrid slots={availability} />}
        </Card>
        ...
        <Card className="glass-panel h-fit rounded-2xl p-5 xl:sticky xl:top-24">
          ...
          <div className="mt-5 space-y-4 text-sm">
            <div className="flex justify-between"><span className="text-zinc-500">Floor</span><span>{room.floor}</span></div>
            <div className="flex justify-between"><span className="text-zinc-500">Capacity</span><span>{room.capacity}</span></div>
            <div className="flex justify-between"><span className="text-zinc-500">Buffer</span><span>{room.bufferTime || 0} min</span></div>
          </div>
          <div className="mt-6">
            <BookingFormModal roomId={room._id} roomName={room.name} defaultDate={date} triggerLabel="Reserve this room" />
          </div>
        </Card>
      </section>
    </div>
